...
def collect_diagnose_findings(
    ...
    findings into plain dicts: rule, scope_type (session|file), scope,
    message, est_wasted_tokens, est_wasted_usd, line (1-based, file findings
    only — editor diagnostics anchor on it). days > 0 keeps only session
    findings whose session started within that window; file findings always
    pass (they describe current files, not time-scoped activity)."""
    valid_sessions = None
    if days > 0:
...
def cmd_diagnose(args: argparse.Namespace) -> int:
    ...
    total_wasted = sum(f["est_wasted_tokens"] for f in findings)
    total_usd = round(sum(f["est_wasted_usd"] for f in findings), 2)
